...
# library() is for importing stuff, like `import` in python. Why not just call it import?
library(argparser)

# if the "argparser" packageVersion < 0.3, then stop happens
...
# commandArgs() returns list of arguments provided while calling this program in command-line
args = commandArgs(trailingOnly=T)

# Add the four strings in the c() function into a 2x2 matrix
...
# Creates an arg.parser object. arg_parser(description, name="...")
# For more info: https://cran.r-project.org/web/packages/argparser/argparser.pdf
p = arg_parser("Run Chicago from input files", name="Rscript runChicago.R")

# add_argument() adds an argument to arg.parser object, here `p` is the arg.parser object 
# add_argument(parser object, argument name, description for the argument)
